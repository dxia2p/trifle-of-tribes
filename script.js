...
let costsArray = [rockmanCost, spearmanCost, bowmanCost, magemanCost, wallCost];

// Draw Background
let cam = new Camera(854, 480, new Vector2(0, 0));
let square = new RectRenderer(
  new Vector2(0, 0),
  BACKGROUND_SIZE,
  BACKGROUND_SIZE,
  "#A6E57A",
...
for (let y = BACKGROUND_SIZE / 2; y > -(BACKGROUND_SIZE / 2); y -= GRID_SIZE) {
  if (!offsetRow) {
    ...
    for (
      let x = BACKGROUND_SIZE / -2 + GRID_SIZE;
      x < BACKGROUND_SIZE / 2;
      x += GRID_SIZE * 2
    ) {
      backgroundSquares.push(
...
let mouseRect = new RectRenderer(
  new Vector2(0, 0),
  GRID_SIZE,
  GRID_SIZE,
  "#FFFFFF",
...
function drawBuildingTemplate(mp) {
  ...
  if (bt.gridWidth % 2 == 0) {
    // OFFSET IT BY HALF THE GRID SIZE IF THE WIDTH IS EVEN
    bt.pos.x -= GRID_SIZE / 2;
  }
  if (bt.gridHeight % 2 == 0) {
...
document.addEventListener("mousedown", (event) => {
  ...
  for (let i = 0; i < placedBuildings.length; i++) {
    if (
      rectangleOverlap(
        buildingTemplates[selectedBuilding].pos,
        buildingTemplates[selectedBuilding].gridWidth * GRID_SIZE,
        buildingTemplates[selectedBuilding].gridHeight * GRID_SIZE,
        placedBuildings[i].pos,
        placedBuildings[i].gridWidth * GRID_SIZE,
        placedBuildings[i].gridHeight * GRID_SIZE
      )
    ) {
      return;
    }
  ...
  switch (selectedBuilding) {
    case 0:
      if (gold >= rockmanCost) {
        ...
        b = new RockThrower(
          new Vector2(
            buildingTemplates[selectedBuilding].pos.x,
            buildingTemplates[selectedBuilding].pos.y
          )
        );
      } else {
...
function rectangleOverlap(
  ...
  let r1TopLeft = new Vector2(
    r1center.x - r1width / 2,
    r1center.y + r1height / 2
  );
  let r1BottomRight = new Vector2(
    r1center.x + r1width / 2,
    r1center.y - r1height / 2
  );

  let r2TopLeft = new Vector2(
    r2center.x - r2width / 2,
    r2center.y + r2height / 2
  );
  let r2BottomRight = new Vector2(
    r2center.x + r2width / 2,
    r2center.y - r2height / 2
  );
...
document.addEventListener("backspace", (deleteBuilding) => {
  ...
  if (
    mousedeletePos.x < -canvas.width / 2 ||
    mousedeletePos.x > canvas.width / 2 ||
    mousedeletePos.y > canvas.height / 2 ||
    mousedeletePos.y < -canvas.height / 2
  ) {
    return;
  }
...
function spawnGoblin() {
  ...
  let coords = new Vector2(
    Math.cos(randAngle) * 1000,
    Math.sin(randAngle) * 1000
  );
  new Goblin(coords);
}